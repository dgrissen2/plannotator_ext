...
tools: Bash, Read, Edit, MultiEdit, Grep, Glob, TodoWrite, Serena
---
...
2. **Execute Validation**
   ```bash
   # TypeScript/Bun validation sequence
   bun run lint
...
## Common Validation Commands for TypeScript/Bun

```bash
bun run lint          # Linting
bun run typecheck     # Type checking (tsc --noEmit)
bun run test          # Run tests (vitest or similar)
